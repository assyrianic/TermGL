...
static inline void TGL_CLEAR_SCREEN(void) {
...
static inline void *TGL_MALLOC(size_t bytes) {
	return calloc(1, bytes);
}

static inline void TGL_FREE(void *ptr) {
	free(ptr);
}
...
enum {
	...
	TGL_Z_BUFFER      = 0x40,
	TGL_OUTPUT_BUFFER = 0x20,
	TGL_DOUBLE_CHARS  = 0x80,
};
 ...
 */
static inline TGL *tgl_init(size_t width, size_t height, const Gradient *gradient);
...
/**
 * Frees and Nullifies a TGL context through pointer.
 * NOTE: pass TGL context like 'tgl_cleanup(&tgl);'
 */
static inline void tgl_cleanup(TGL **tgl_ref);
...
static inline void tgl_puts(TGL *tgl, int x, int y, const char *str, uint8_t color);
...
struct TGL {
	...
	size_t          frame_size, output_buffer_size;
	size_t          width, height;
	int             max_x, max_y;
	uint8_t         settings;
	bool            is_z_buffered : 1;
};
...
/// Replacing macros with fixed-type
/// because only ints are using MIN and MAX
...
static inline void tgl_add3s(const float vec1[3], const float summand, float res[3]) {
	for( size_t i=0; i < 3; i++ ) {
		res[i] = vec1[i] + summand;
	}
}

static inline void tgl_sub3s(const float vec1[3], const float subtrahend, float res[3]) {
	for( size_t i=0; i < 3; i++ ) {
		res[i] = vec1[i] - subtrahend;
	}
}
...
enum {
	TGL_CULL_BIT      = 0x01,
	TGL_CULL_FACE_BIT = 0x01,
	TGL_WINDING_BIT   = 0x02,
};

static inline float MAP_COORD(const float half, const float val) {
...
#pragma GCC unroll 3
	for( size_t i=0; i < 3; i++ ) {
		res[i] = tgl_dot43(mat[i], vec);
	}
	
	const float w = tgl_dot43(mat[3], vec);
	if (w != 0.f) {
		#pragma GCC unroll 3
		for( size_t i=0; i < 3; i++ ) {
			res[i] *= 1.f / w;
...
static inline void tgl3d_camera(TGL *tgl, float fov, float near, float far)
{
	...
	const TGLMat projection = {
		{s * tgl3d->aspect_ratio, 0.f, 0.f, 0.f},
		{0.f, s, 0.f, 0.f},
		{0.f, 0.f, -(far + near) * a, 2.f * far * near * a},
		{0.f, 0.f, 1.f, 0.f},
	};
	memcpy(tgl->tgl3d->projection, projection, sizeof projection);
}

static inline void tgl3d_transform_rotate(TGLTransform *transform, float x, float y, float z)
{
	const TGLMat rotate = ROTATION_MATRIX(x, y, z);
	memcpy(transform->rotate, rotate, sizeof rotate);
}

static inline void tgl3d_transform_scale(TGLTransform *transform, float x, float y, float z)
{
	const TGLMat scale = SCALE_MATRIX(x, y, z);
	memcpy(transform->scale, scale, sizeof scale);
}

static inline void tgl3d_transform_translate(TGLTransform *transform, float x, float y, float z)
{
	const TGLMat translate = TRANSLATE_MATRIX(x, y, z);
	memcpy(transform->translate, translate, sizeof translate);
}
...
static inline size_t clip_triangle_plane(const TGLVec3 normal, TGLTriangle *in, TGLTriangle out[2])
{
	TGLVec3 di = {
	...
	size_t n_inside = 0, n_outside = 0;
	size_t inside[3] = {0}, outside[3] = {0};
#pragma GCC unroll 3
	for (size_t i = 0; i < 3; i++) {
		if (di[i] >= 0.f)
			inside[n_inside++] = i;
...
static inline void tgl3d_shader(TGL *tgl, TGLTriangle *in, uint8_t color, bool fill, void *data, void intermediate_shader(TGLTriangle*, void*))
{
	...
	memcpy(out.intensity, in->intensity, sizeof in->intensity);
	...
	/// the size of this buffer assumes a pathological case which is probably impossible
	TGLTriangle trig_buffer[127];
	memcpy(&trig_buffer[0], &out, sizeof out);
	size_t buffer_offset = 0;
	size_t n_cur_stage   = 1;
	for (size_t p = 0; p < (sizeof clip_plane_normals / sizeof clip_plane_normals[0]); p++) {
		size_t n_next_stage = 0;
		for (size_t i = 0; i < n_cur_stage; i++) {
...
static inline void tgl3d_cull_face(TGL *tgl, uint8_t settings) {
	TGL3D *tgl3d = tgl->tgl3d;
	tgl3d->settings = (tgl3d->settings & ~TGL_CULL_BIT) | (XOR(settings & TGL_CULL_FACE_BIT, settings & TGL_WINDING_BIT) ? TGL_CULL_BIT: 0);
}
...
static inline void SET_PIXEL_RAW(TGL *tgl, int x, int y, char v_char_, uint8_t color_) {
...
static inline void SET_PIXEL(TGL *tgl, int x, int y, float z, char v_char_, uint8_t color_) {
	if (!tgl->is_z_buffered) {
...
static inline char INTENSITY_TO_CHAR(TGL *tgl, uint8_t intensity) {
	return tgl->gradient->grad[(tgl->gradient->length * intensity) / 256u];
}
...
static inline void tgl_clear(TGL *tgl, const uint8_t buffers) {
	if (buffers & TGL_FRAME_BUFFER) {
		...
		for (size_t i = 0; i < tgl->frame_size; i++) {
	...
	if (buffers & TGL_OUTPUT_BUFFER) {
		memset(tgl->output_buffer, 0, tgl->output_buffer_size * sizeof *tgl->output_buffer);
	}
}

static inline TGL *tgl_init(const size_t width, const size_t height, const Gradient *gradient)
{
	...
	*tgl = (TGL) {
	...
	tgl->frame_buffer = TGL_MALLOC(tgl->frame_size * sizeof *tgl->frame_buffer);
	if( tgl->frame_buffer==NULL ) {
		tgl_cleanup(&tgl);
		return tgl;
	}
...
static inline void tgl_flush(TGL *tgl)
{
	...
	if (tgl->output_buffer_size) {
		...
		for (size_t row = 0; row < tgl->height; row++) {
			for (size_t col = 0; col < tgl->width; col++) {
...
static inline void tgl_puts(TGL *tgl, int x, int y, const char *str, uint8_t color) {
	/// we ASSUME 'str' is NOT NULL...
	itgl_clip(tgl, &x, &y);
	for( size_t i=0; str[i] != 0; i++ ) {
		SET_PIXEL_RAW(tgl, x, y, str[i], color);
	}
}
...
LBL_NEXT2:
		...
		const int vi0   = ((y - y0) * i1 + (y1 - y) * i0) / (y1 - y0);
		const int vi1   = ((y - y0) * i2 + (y2 - y) * i0) / (y2 - y0);
		const float vz0 = ((y - y0) * z1 + (y1 - y) * z0) / (y1 - y0);
		const float vz1 = ((y - y0) * z1 + (y1 - y) * z0) / (y1 - y0);
		if (t0x < t1x)
...
LBL_NEXT4:
		...
		if (y1 != y2) {
			const int vi0   = ((y - y0) * i2 + (y2 - y) * i0) / (y2 - y0);
			const int vi1   = ((y - y1) * i2 + (y2 - y) * i1) / (y2 - y1);
			const float vz0 = ((y - y0) * z2 + (y2 - y) * z0) / (y2 - y0);
			const float vz1 = ((y - y1) * z2 + (y2 - y) * z1) / (y2 - y1);
			if (t1x < t0x)
				itgl_horiz_line(tgl, minx, vz0, vi0, maxx, vz1, vi1, y, color);
			else
				itgl_horiz_line(tgl, minx, vz1, vi1, maxx, vz0, vi0, y, color);
...
static inline void tgl_enable(TGL *tgl, uint8_t settings) {
	...
	if (settings & TGL_Z_BUFFER) {
		tgl->is_z_buffered = true;
		tgl->z_buffer = TGL_MALLOC(sizeof *tgl->z_buffer * tgl->frame_size);
		tgl_clear(tgl, TGL_Z_BUFFER);
	}
	if (settings & TGL_OUTPUT_BUFFER) {
		tgl->output_buffer_size = 14 * tgl->frame_size + tgl->height + 1;
		tgl->output_buffer = TGL_MALLOC(tgl->output_buffer_size * sizeof *tgl->output_buffer);
